<template>
  <div class="content-wrapper">
                ...
                <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                  <el-tab-pane label="全部住户" name="first">
                    <div class="form-horizontal">
                      <div class="col-lg-2 pl-0">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-input clearable style="width:100%" placeholder="搜索姓名/手机号码" v-model="PersonListForm.queryContent"></el-input>
                          </div>
                        </div>
                      </div>
                      <div class="col-lg-2">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-cascader
                                ...
                                v-model="PersonListForm.houseIds"
                      ...
                      <div class="col-lg-2">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-select clearable v-model="PersonListForm.personType" placeholder="住户类型">
                              <el-option
                    ...
                    <div id="toolbar" class=" btn-group-box" style="width:100%">
                      <div class="btn-group" data-toggle="buttons">
                        ...
                        <button id="btn_delete" type="button" class="btn btn-default" @click="batchRenewal(1)">
                          <span class="glyphicon glyphicon-time" aria-hidden="true"></span> 批量续期
                        </button>
                      </div>
                    ...
                    >
                     <el-table-column slot="operate1" label="住户类型" align="center" min-width="120" prop="personSubtype" sortable class="nice">
                      ...
                      <el-table-column slot="operate2" label="住宅信息" align="center" min-width="120" prop="houseAddress" sortable>
                         <template slot-scope="scope">
                          <div v-for="(item,index) in scope.row.houseList" :key="index">
                      ...
                      <el-table-column slot="operate3" label="授权状态" align="center" width="120" prop="grantSuccessNum" sortable>
                         <template slot-scope="scope">
                          <!-- <span class="see-files" @click="goView(scope.row.id,scope.row.personName)">查看</span> -->
                          <div class="grant-box" @click="goView(scope.row.personId,scope.row.personName)" v-if="scope.row.grantTotalNum!=0&&scope.row.grantSuccessNum>=scope.row.grantTotalNum">
                            <div class="grant-color-s"></div>
                            <span class="grant-num-s">{{scope.row.grantSuccessNum}}/{{scope.row.grantTotalNum}}</span>
                          </div>
                          <div class="grant-box" @click="goView(scope.row.personId,scope.row.personName)" v-if="scope.row.grantTotalNum!=0&&scope.row.grantSuccessNum==0">
                            <div class="grant-color-f"></div>
                            <span class="grant-num-f">{{scope.row.grantSuccessNum}}/{{scope.row.grantTotalNum}}</span>
                          </div>
                          <div class="grant-box" @click="goView(scope.row.personId,scope.row.personName)" v-if="scope.row.grantTotalNum!=0&&scope.row.grantSuccessNum<scope.row.grantTotalNum&&scope.row.grantSuccessNum!=0">
                            <div class="grant-color-half"></div>
                            <span class="grant-num-half">{{scope.row.grantSuccessNum}}/{{scope.row.grantTotalNum}}</span>
                          </div>
                        </template>
                     </el-table-column>
                     <el-table-column slot="operate5" label="过期时间" align="center" min-width="120" prop="effectiveTime" sortable>
                         <template slot-scope="scope">
                          <div v-for="(item,index) in scope.row.houseList" :key="index">
                            <span>{{item.effectiveTime.substring(0,10)}}</span>                          
                          </div>
                        </template>
                     </el-table-column>
                        ...
                        <template slot-scope="scope">
                           <el-row>
                             ...
                             <i class="el-icon-delete">删除</i> -->
                               <el-button title="编辑" type="primary" icon="el-icon-edit"  size="mini" @click="goEdit(scope.row)"></el-button>
                              <el-button title="删除" type="danger" icon="el-icon-delete"  size="mini" @click="deleteOwner(scope.row.personId)"></el-button>
                            </el-row>
                        </template>
                    </el-table-column>
                    </table-cmp>
                  ...
                  <el-tab-pane label="住户审核" name="second">
                    ...
                    <div class="form-horizontal">
                     ...
                     <div id="toolbar" class=" btn-group-box" style="width:100%">
                       <div class="btn-group" data-toggle="buttons">
                        <button id="btn_delete" type="button" class="btn btn-default" @click="batchAudit">
                          批量审核
                        </button>
                        <button id="btn_delete" type="button" class="btn btn-default" @click="exportBtn">
                          <span class="glyphicon glyphicon-arrow-down" aria-hidden="true"></span>导出
                    ...
                    <el-table-column slot="operate" label="操作" align="center" width="160">
                        <template slot-scope="scope">
                          <!-- <el-row v-if="scope.row.bindStatus==0||scope.row.bindStatus==1||(scope.row.personSubtype==1&&scope.row.bindStatus==0)"> -->
                            <el-row>
                              <el-button v-if="(scope.row.bindStatus==0 && !scope.row.identityAudit) || scope.row.bindStatus==1" title="编辑" type="primary" icon="el-icon-edit"  size="mini" @click="goExamine(scope.row)"></el-button>
                              <el-button v-if="(scope.row.bindStatus==0 && !scope.row.identityAudit) || scope.row.bindStatus==1" title="审核" type="primary" icon="el-icon-s-check"  size="mini" @click="goCheck(scope.row)"></el-button>
                            </el-row>
                        </template>
                    </el-table-column>
                    </table-bmp>
                  ...
                  <el-tab-pane label="续约住户" name="third">
                    <div class="form-horizontal">
                      <div class="col-lg-2 pl-0">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-input clearable style="width:100%" placeholder="搜索姓名/手机号码" v-model="PersonListForm2.queryContent"></el-input>
                          </div>
                        </div>
                      </div>
                      <div class="col-lg-2">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-cascader
                                ...
                                clearable
                                placeholder="楼栋/单元/房间"
                                v-model="PersonListForm2.houseIds"
                                :options="cascaderData"
                                :props="groupProps"
                                 @visible-change="elCascaderOnlick"
                                @expand-change="elCascaderOnlick"
                                @change="cascaderChange1"></el-cascader>
                          </div>
                        </div>
                      </div>
                      <div class="col-lg-2">
                        <div class="form-group">
                          <div class="col-md-12">
                            <el-select clearable v-model="PersonListForm2.personType" placeholder="住户类型">
                              <el-option
                                v-for="item in personTypeList"
                                :key="item.value"
                                :label="item.label"
                      ...
                      <div class="col-lg-4  pr-0">
                        <div class="btn-group">
                          <button id="btn_device_search" type="button" class="btn btn-primary" @click="getRenewalList">
                            <span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查询
                          </button>
                          <button id="btn_device_reset" type="button" class="btn btn-default" @click="resetBtn2">
                            <span class="fa fa-reply" aria-hidden="true"></span> 重置
                          </button>
                          <button id="btn_device_reset" type="button" class="btn btn-default" @click="batchRenewal(2)">
                            <span class="glyphicon glyphicon-time" aria-hidden="true"></span> 批量续约
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                     <el-table-column slot="operate1" label="住户类型" align="center" min-width="120" prop="personSubtype" sortable class="nice">
                        <template slot-scope="scope">
                          <div>
                            <span v-if="scope.row.personResidentHouseListVo.personSubtype == 1">户主</span>
                            <span v-if="scope.row.personResidentHouseListVo.personSubtype == 2">家庭成员</span>
                            <span v-if="scope.row.personResidentHouseListVo.personSubtype == 3">租客</span>
                            <span v-if="scope.row.personResidentHouseListVo.personSubtype == 5">其他</span>
                            <!-- <span v-if="item.personSubtype==4">物业员工</span>
                            <span v-if="item.personSubtype==5">访客</span> -->
                          </div>
                        </template>
                     </el-table-column>
                      <el-table-column slot="operate2" label="住宅信息" align="center" prop="houseAddress" min-width="120" sortable>
                         <template slot-scope="scope">
                          <div>
                            <span>{{scope.row.personResidentHouseListVo.houseAddress}}</span>                          
                          </div>
                        </template>
                     </el-table-column>
                     <el-table-column slot="operate2" label="有效时间" align="center" prop="effectiveTime" min-width="120" sortable>
                         <template slot-scope="scope">
                          <div>
                            <span>{{scope.row.personResidentHouseListVo.effectiveTime.substring(0,10)}}</span>                          
                          </div>
                        </template>
                     </el-table-column>
                    <el-table-column slot="operate" label="操作" align="center" width="100">
                        <template slot-scope="scope">
                           <el-row>
                             <!-- <i class="el-icon-edit">IC卡</i>
                             <i class="el-icon-delete">删除</i> -->
                               <el-button title="编辑" type="primary" icon="el-icon-edit"  size="mini" @click="goRenewal(scope.row.personResidentHouseListVo.personDeviceBingId)"></el-button>
                            </el-row>
                        </template>
                    </el-table-column>
                    </table-cmp>
    ...
    <el-dialog
     ...
     <el-dialog
        :title="timesTitle"
        :visible.sync="dialogVisibleRenewalTime"
        width="25%">
        <div class="row">
           <div class="col-sm-12">
             <label for="name" class="col-sm-3 control-label text-right pt-10"><span>续约时间</span>{{headTitle}}</label>
             <div class="col-sm-7">
                <el-date-picker
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisibleRenewalTime = false">取 消</el-button>
          <el-button type="primary" @click="setRenewalTime">确 定</el-button>
        </span>
      </el-dialog>
