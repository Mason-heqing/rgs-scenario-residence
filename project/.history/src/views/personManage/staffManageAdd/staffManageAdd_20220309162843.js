...
export default {
  ...
  mounted() {
    ...
    if(this.$route.query.type==2){//编辑
      ...
      console.log("编辑室获取房屋数据:",this.addPeople.deviceGroupList);
      this.addPeople.deviceGroupList=this.addPeople.deviceGroupList;
      // this.appList = this.addPeople.deviceGroupList;
  ...
  picClipping(img){
      ...
      res => {
        if(res.code==200){
          ...
          if(this.$route.query.type==2){
             console.log("获取fw:",this.data);
             let queryArr = this.addPeople.deviceGroupList;
             console.log("获取123:",queryArr);
             for(let i = 0;i<queryArr.length;i++){
              this.parentHouseIdAdd(queryArr[i],this.data,this.appList);
              console.log("返回的数据呢:",this.appList);
             }
          }
    ...
    checkChange(data,checked, deep){
      ...
      // console.log("选中数据:",[...new Set(parentData)]);   
      // console.log("选中数据1:",this.$refs.tree.getCheckedKeys());
      // let allData = [...new Set(parentData)]  
      this.addParent();
      this.addPeople.deviceGroupList = this.$refs.tree.getCheckedKeys();
      // console.log("选中数据:",this.addPeople.deviceGroupList)     
    },
  ...
  //   //编辑时房屋数据处理
    parentHouseIdAdd(value,data,arr){
      for( var i  = 0;i < data.length;i++){
        var mod = data[i]
        console.log('这是什么---->',mod.id,value);
        if(mod.id == value) {
          // console.log('这是什么---->',mod.id);
          alert(111);
          arr.push(mod.id)
          // if(this.noRepeat(arr,mod.id)) {
          //   // console.log("qqqq>>>>>>,",mod.id);
          //   arr.push(mod.id)
          // }
          return true;
      }else {
        if(mod.groupList) {
          var bool = this.parentHouseIdAdd(value, mod.groupList, arr)
          if(bool) {
            arr.push(mod.id)
            // if (this.noRepeat(arr, mod.id)) {
            //   arr.push(mod.id)
    ...
    addParent(){
      ...
      let parentData = [...new Set(arr)];
      let secletData = this.$refs.tree.getCheckedKeys();
      secletData.forEach((item) => {
        parentData.push(item);
      })
      parentData.unshift(this.data[0].id);
      this.appList = [...new Set(parentData)];
    },
    ...
    addBtn(fromMsg){
      ...
      this.$refs[fromMsg].validate((valid) => {
        if(valid){
          ...
          let json = {};
          for(var i in this.addPeople){
             json[i] = this.addPeople[i];
          }
          json.deviceGroupList = this.appList;
          // console.log(json);
          this.utils.http({
