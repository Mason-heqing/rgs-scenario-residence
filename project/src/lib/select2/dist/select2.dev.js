  ...
  var S2 = function () {
    ...
    S2.define('select2/selection/search', ['jquery', '../utils', '../keys'], function ($, Utils, KEYS) {
      ...
      Search.prototype.bind = function (decorated, container, $container) {
        ...
        var disableInputEvents = msie && msie <= 11; // Worka for browsers which do not support the `input` event
        // This will prevent double-triggering of events for browsers which support
        // both the `keyup` and `input` events.

        this.$selection.on('input.searchcheck', '.select2-search--inline', function (evt) {
          // IE will trigger the `input` event when a placeholder is used on a
          // search box. To get a this issue, we are forced to ignore all
    ...
    S2.define('select2/dropdown/search', ['jquery', '../utils'], function ($, Utils) {
      ...
      Search.prototype.bind = function (decorated, container, $container) {
        ...
        this.$search.on('keydown', function (evt) {
          self.trigger('keypress', evt);
          self._keyUpPrevented = evt.isDefaultPrevented();
        }); // Worka for browsers which do not support the `input` event
        // This will prevent double-triggering of events for browsers which support
        // both the `keyup` and `input` events.
