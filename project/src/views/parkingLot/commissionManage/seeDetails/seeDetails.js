...
export default {
  ...
  mounted() {
    ...
    if(this.$route.query.type==1){//开户返佣明细
        ...
        this.tableLabel=[    
            { label: '停车场', param: 'parkName', align: 'left',isshow:true, sortable: true,width:'150' },
            { label: '月日均笔数', param: 'avgMonthNum', align: 'left',isshow:true, sortable: true,width:'150' },
            { label: '计算阶梯', param: 'ladder', align: 'left',isshow:true, width:'120',sortable: true },
            { label: '返佣金额(元)', param: 'rewardAmount', align: 'left',isshow:true, width:'120',sortable: true,render(h) {
              if(0 != h.rewardAmount){
         ...
         this.myName = '支付返佣明细';
         console.log("列表信息-------->",JSON.parse(this.utils.getStore('seeDetailsData')).reconciliationMonth)
         this.dataForm.reconMonth = JSON.parse(this.utils.getStore('seeDetailsData')).reconciliationMonth.replace(/-/g,'');
         console.log("月份-->",this.dataForm.reconMonth)
        //  this.totalName = '支付返佣总金额（元)';
        this.totalAmount = JSON.parse(this.utils.getStore('seeDetailsData')).aliCommissionAmount + JSON.parse(this.utils.getStore('seeDetailsData')).wechatCommissionAmount;
         ...
         this.tableLabel=[    
            ...
            { label: '停车场', param: 'parkName', align: 'left',isshow:true, sortable: true,width:'150' },
            { label: '商户ID', param: 'merchatId', align: 'left',isshow:true, width:'120',sortable: true },
            ...
            { label: '交易金额(元)', param: 'transAmount', align: 'left',isshow:true, width:'120',sortable: true,render(h) {
              if(0 != h.transAmount){
            ...
            { label: '返佣金额(元)', param: 'refundAmount', align: 'left',isshow:true, width:'120',sortable: true,render(h) {
              if(0 != h.refundAmount){
   ...
   getTableData(url){
    ...
    handleSizeChange(val) {
      ...
      if(this.$route.query.type==1){
        ...
        this.getTableData('/manage/export/reward/exportPayDeail');
      }else{
        this.getTableData('/manage/reward/pageMaterialDeail');
      }
    },
    ...
    sendExport(){
      let url = '';
      if(this.$route.query.type==1){
        url = '/manage/export/reward/exportAccountDeail';
      }else if(this.$route.query.type==2){
        url = '/manage/export/reward/exportPayDeail';
      }else{
