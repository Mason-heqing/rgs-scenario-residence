         ...
         <div class="box box-primary">
              <div class="box-body" v-loading='loading'>
                    ...
                    <el-table-column slot="operate" label="操作" align="center" width="360">
                        <template slot-scope="scope">
                           <el-row>
                              <el-button title="编辑" type="primary" icon="el-icon-edit" round  size="mini" @click="editDevice(scope.row.id,scope.row.online)"></el-button>
                              <el-button title="监控" type="primary" icon="el-icon-view" round  size="mini" @click="getVideoWebsocket(scope.row.id,scope.row.online)"></el-button>
                              <el-button title="重启" type="primary" icon="el-icon-switch-button" round  size="mini" @click="restartDevice(scope.row.id,scope.row.online)"></el-button>
                              <el-button title="重新授权" type="primary" icon="el-icon-refresh" round  size="mini" @click="reAuth(scope.row.id,scope.row.online)"></el-button>
                              <el-button title="开门" type="primary" icon="el-icon-key" round  size="mini" @click="openDoorRequst(scope.row.id,scope.row.online)"></el-button>
                              <el-button title="删除" type="danger" icon="el-icon-delete" round size="mini" @click="deleteDevice(scope.row.id)"></el-button>
                            </el-row>
                        </template>
                    </el-table-column>
                    </table-cmp>
