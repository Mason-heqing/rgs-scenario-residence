...
export default {
  ...
  data() {
    ...
    FailedList:[],//升级失败提示数据
    isUping:false,
    parameterLoading:false,//参数加载状态
    tips:'',//加载提示文字
    authorizeLoading:false,//授权数据加载状态
    }
  },
  computed: {
    appId:{
  ...
  mounted() {
    if(this.appId){
      ...
      this.getLeftData();
      this.getDataTable();
      window.localStorage.setItem('prjName',this.projectName)
      console.log(window.localStorage.getItem('prjName'))
      // this.initWebSocket();
    }
  },
  methods: {
    ...
    getPersonAuth(row){
      ...
      console.log(row,'odidd')
      this.utils.http({
          url: "/device/logInto",
          method: "GET",
          params: {deviceId:row.deviceId,personId:row.personId}
        },res => {
          if(res.code==200){
            console.log(res,'****---')
    ...
    webSocketMessage(event) {
      try {
        console.log("websocket返回数据:",event);
    ...
    toggleTable(index) {
      ...
      this.spanTag = index
      this.authForm.authStatus = index
      this.authForm.page = 1;
      this.authForm.pageSize = 10;
      this.utils.http({
    ...
    powerOperation(row){
      this.utils.http({
        ...
        data:{
          ...
          deviceIds:this.authForm.deviceId
        }
    ...
    getDataTable(){
      ...
      res => {
        ...
        if(res.code==200){
          ...
          //   this.projectName = res.data.records[0].groupName
          // }else{
          //   this.projectName = ''
          // }
        }
    ...
    seeAuthPeople(row){
      console.log(row,'row')
      this.authorizeLoading = true;
      this.OdeviceId = row.id
      this.dialogVisible=true;
      this.authForm.deviceId=this.OdeviceId;
      this.authForm.authStatus = this.spanTag
      this.utils.http({
        url: "/device/personPage",
        method: "POST",
        data: this.authForm
    ...
    handleSizeChange1(val) {
      ...
      this.seeAuthPeople({id:this.OdeviceId});
    },
    ...
    getDeviceData(id){
      ...
      res => {
        if(res.code==200){ 
          ...
          if(jsonData.data){
            this.paramsSet=jsonData.data;
            this.currentTab=jsonData.data[0];
          }else{
            this.paramsSet=jsonData;
